...
Hazel aims at helping you implement __Continuous Integration__ and __Continuous
Delivery__ by providing you with an platform that will keep track of your applications,
...
also support _webhooks_ so that you can deploy right after the CI runs.
...
a little more flexibility to fit in more use cases. Plus instead is being a monolith
like those last two it is composes of multiple small applications working together all
...
The end goal of Hazel is enabling _DevOps_ and shipping continuously and efficiently
a _SOA_. _Microservices_ makes you code easy to write in those decoupled but what's
in between of them and the infrastructure related to them gets more complex compared
to a monolith.
...
dynamic _DNS_ updating, rolling updates, supporting more than 5-10 applications and other
stacks than _Node.js_ is not needed for basic usage and will come in time.

### Phase 1 : Sketching things out, _proof-of-concept_
...
_webhooks_ to create new builds on demand.
...
- `status=waiting` Polls S3 waiting for a tar to be uploaded with the application
   code to be uploaded
- `status=building` Fetches up to date environment variables, parses and outputs
  all templates defined by the stack to have dynamic configuration, runs **packer** (from
...
- `status=built` This is the status after `building`, at this point the **builder**
  ...
  of the normal amount of servers using the image, this way updates roll forward
...
app will simply run terraform with the newly build **ami**'s when it sees un-deployed
builds.

#### Environment (vars) API

Environment variables being stored centrally is a huge plus as you can have a web
UI to easily change them and trigger deploys, this service is a simple JSON over
HTTP API that only holds a few endpoints for applications and their configuration variables.
...
This UI allow editing environment variables for one application. It's not directly in
the **Web UI** as many more feature needing web interfaces are coming and if
...
the API service as it's the one that holds all the configuration, metadata and state and
would be able to:

- Trigger builds for an application
- See a list of builds for an application and their state
- See a specific build logs
...
Let's be really "hacky" and use only the filesystem, ok S3. After all _Deployinator_
...
that happened more than X minutes ago, of for degrading metrics past a certain
...
### Phase 3 : We give you more than Heroku
...
- Alerts for unhealthy core services
...
Until now it was OK to use Logentries directly on machines, now we might want to
...
a product, even to developers, than CLIs do. But, we still need one for efficiency
and scripting sake.
...
is not decided yet. Maybe Graphite, maybe InfluxDB, maybe something else. But
one thing is sure, it wont be written specifically for Hazel. It would still imply
...
The scale of events will grow really faster with microservices interacting with
each other that with one monolith

#### Better Monitoring Collector (more integrations)

#### More complex overseer, builder, log aggregation, monitoring
...
__This part (the design) is constantly changing, reading the code is more acurate__
